...
def find_largest(line: str) -> int:
    """ Return the largest val in line, which is a whitespace-delimited string
    of integers that each end with a '.'.
    >>> find_largest('1. 3. 5. 3.')
    5
    """
    # the largest val seen so far
    largest = -1
...
def process_file(reader: TextIO) -> int:
    ...
    return the largest val after the header. there may be multiple pieces
    ...
    # the largst val so far is the largest on this first line of data
