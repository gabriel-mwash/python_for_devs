...
def find_largest(line: str) -> int:
    """ Return the largest value in line, which is a whitespace-delimited string of
    integers that each end with a '.'.
...
def process_file(reader: TextIO) -> int:
    ...
    Return the largest value after the header. There may be mutliple pieces
    of data on eachline.
    ...
    # check the restof the lines for larger values
    remaining_content = reader.readlines()
